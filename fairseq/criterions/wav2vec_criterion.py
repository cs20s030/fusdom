...
#from fairseq.criterions.afd import AFD
...
@register_criterion("wav2vec_distill", dataclass=Wav2VecCriterionConfig)
class Wav2vecCriterion(FairseqCriterion):
    def __init__(self, task, infonce=False, loss_weights=None, log_keys=None):
        super().__init__(task)
        self.infonce = infonce
    ...
    def forward(self, model, sample, reduce=True):
        ...
        3) logging outputs to display while training
        """
        #x = sample["net_input"]['source']
        #print(x['source'])
        #exit()
        #padding_mask = torch.BoolTensor(x.size(1)).fill_(False).unsqueeze(0)
        #with torch.inference_mode():
            #model_ops = self.teacher.extract_features(x, padding_mask)
        #print(model_ops.keys())
        #exit()
        net_output = model(**sample["net_input"])
        logits = model.get_logits(net_output).float()
        target = model.get_targets(sample, net_output)
        ...
        loss_teacher = net_output['distill_loss']
        scale_factor = net_output['distill_factor']
        ...
        loss_combined = scale_factor*loss + loss_teacher #0.01 weight for specific loss

        logging_output = {
            "loss_combined": loss_combined.item() if (reduce and not self.xla) else loss_combined.detach(),     
            "loss": loss.item() if (reduce and not self.xla) else loss.detach(),
            "ntokens": sample_size,
            "nsentences": sample["id"].numel(),
            "sample_size": sample_size,
            "teacher_loss": loss_teacher.item() if (reduce and not self.xla) else loss_teacher.detach(),
        }
        ...
        return loss_combined, sample_size, logging_output
